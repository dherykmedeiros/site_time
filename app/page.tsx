...
export default async function HomePage({
  ...
  return (
    ...
    >
      ...
      {session && (
        <div className="relative z-50 bg-[#10b981] border-b-2 border-black px-4 py-2.5 text-center text-[10px] font-black uppercase tracking-widest text-[#090d0f] shadow-sm font-mono">
      ...
      <main className="mx-auto mt-6 max-w-6xl px-4 sm:px-6 lg:px-8 space-y-24">
        ...
        {/* Estrelas da Temporada (Hall of Fame) - Asymmetric layout */}
        <section id="destaques" className="scroll-mt-24 space-y-6">
          <div className="mb-6 flex items-end justify-between gap-3 border-b border-white/5 pb-4">
            ...
            <span className="hidden sm:inline-block text-[10px] font-bold uppercase tracking-wider text-slate-500 font-mono">
              Métricas Oficiais
            </span>
          </div>
          ...
          <div className="space-y-8">
            ...
            <div className="space-y-4">
              ...
              {finishedMatches.length === 0 ? (
                <div className="rounded-none border-2 border-slate-800 p-12 text-center text-slate-500 text-sm font-medium bg-black/20 border-dashed font-mono">
                ...
                <div className="border-2 border-slate-800 rounded-none bg-[#0b0f11] overflow-hidden divide-y-2 divide-slate-800 shadow-[4px_4px_0px_0px_#000]">
                  {finishedMatches.map((match) => {
                    ...
                    return (
                      <div key={match.id} className="p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition-colors hover:bg-white/[0.01] font-mono">
            ...
            {/* LEVEL 3: Outros Jogos Agendados */}
        ...
        {stats.activeSeason && (
          <section id="classificacao" className="scroll-mt-24 space-y-6">
            <div className="mb-6 flex items-end justify-between gap-3 border-b border-white/5 pb-4">
              <div>
                <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[var(--brand)] font-mono">Classificação</p>
                <h2 className="text-3xl font-black uppercase text-white tracking-tight font-mono mt-1">Tabela Individual da Temporada</h2>
                <p className="text-xs text-slate-400 font-mono mt-1">Campeonato ativo: {stats.activeSeason.name}</p>
              </div>
              <span className="hidden sm:inline-block text-[10px] font-bold uppercase tracking-wider text-slate-500 font-mono">
                Ranking Interno
              </span>
            ...
            {stats.activeSeasonStandings.length > 0 ? (
              <div className="border-2 border-slate-800 rounded-none bg-[#0b0f11] overflow-hidden shadow-[4px_4px_0px_0px_#000]">
                <div className="overflow-x-auto">
                  <table className="w-full text-left text-sm divide-y-2 divide-slate-800 font-mono">
                    <thead>
                      <tr className="bg-black/40">
                        <th className="px-5 py-4 font-black text-slate-400 text-[10px] uppercase tracking-[0.15em]">Pos</th>
                        <th className="px-5 py-4 font-black text-slate-400 text-[10px] uppercase tracking-[0.15em]">Atleta</th>
                        <th className="px-4 py-4 text-center font-black text-slate-400 text-[10px] uppercase tracking-[0.15em]">Jogos</th>
                        <th className="px-4 py-4 text-center font-black text-slate-400 text-[10px] uppercase tracking-[0.15em]">Vitórias</th>
                        <th className="px-4 py-4 text-center font-black text-slate-400 text-[10px] uppercase tracking-[0.15em]">Empates</th>
                        <th className="px-4 py-4 text-center font-black text-slate-400 text-[10px] uppercase tracking-[0.15em]">Derrotas</th>
                        <th className="px-4 py-4 text-center font-black text-slate-400 text-[10px] uppercase tracking-[0.15em]">SG</th>
                      ...
                      {stats.activeSeasonStandings.map((row, idx) => (
                        ...
                        >
                          ...
                          <td className="px-5 py-4">
                            <p className="font-extrabold text-white uppercase">{row.playerName}</p>
                            <p className="text-[10px] text-slate-500 mt-0.5">Camisa #{row.shirtNumber ?? "—"}</p>
                          </td>
                          <td className="px-4 py-4 text-center font-bold text-white">{row.played}</td>
                          <td className="px-4 py-4 text-center font-bold text-[var(--team-primary)]">{row.won}</td>
                          <td className="px-4 py-4 text-center font-bold text-slate-500">{row.drawn}</td>
                          <td className="px-4 py-4 text-center font-bold text-rose-500">{row.lost}</td>
                          <td className="px-4 py-4 text-center font-bold text-white">
                            {row.goalDiff > 0 ? `+${row.goalDiff}` : row.goalDiff}
                          </td>
                          <td className="px-5 py-4 text-center text-lg font-black text-[var(--team-primary)]">{row.points}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              <div className="border-2 border-slate-800 rounded-none bg-black/20 p-12 text-center text-slate-500 text-sm font-medium border-dashed font-mono">
        ...
        {/* ELENCO OFICIAL (SQUAD DOSSIER) - Panini Collectible Trading Cards */}
        <section id="elenco" className="scroll-mt-24 space-y-6">
          ...
          {team.players.length === 0 ? (
            <div className="rounded-none border-2 border-slate-800 p-12 text-center text-slate-500 text-sm font-medium border-dashed font-mono">
        ...
        {(team.openMatchSlots.length > 0 || hasDiscoveryInfo) && (
          <section id="agenda-aberta" className="scroll-mt-24 rounded-none border-2 border-slate-800 bg-[#0b0f11] p-6 sm:p-8 space-y-6 shadow-[6px_6px_0px_0px_#000]">
            ...
            {team.openMatchSlots.length > 0 ? (
                ...
                {team.openMatchSlots.map((slot) => (
                  <article key={slot.id} className="rounded-none border-2 border-slate-800 bg-[#090d0f] p-6 flex flex-col justify-between shadow-[4px_4px_0px_0px_#000] hover:translate-x-[-2px] hover:translate-y-[-2px] hover:shadow-[6px_6px_0px_0px_var(--team-primary)] transition-all duration-200 group">
                    <div className="space-y-2">
                      ...
                      {slot.notes && (
                        <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wide border-t border-slate-800/40 pt-2 font-mono mt-2">
                          [NOTA]: {slot.notes}
                        </p>
                      )}
                    </div>
                    <Link
                      ...
                      className="mt-6 inline-flex min-h-10 items-center justify-center rounded-none border-2 border-[var(--brand)] text-[var(--brand)] hover:bg-[var(--brand)] hover:text-[#090d0f] text-[10px] font-black uppercase tracking-wider px-6 transition-all duration-150 shadow-[2px_2px_0px_0px_rgba(0,0,0,0.5)] active:translate-x-0 active:translate-y-0"
                    >
              ...
              <p className="text-xs text-slate-400 font-bold uppercase tracking-wide font-mono">
          ...
          <div className="rounded-none border-2 border-slate-800 bg-[#0b0f11] p-6 sm:p-8 flex flex-col space-y-6 shadow-[6px_6px_0px_0px_#000]">
            ...
            {team.publicDirectoryOptIn ? (
              ...
              <div className="rounded-none border border-slate-800 bg-black/40 p-8 text-center text-slate-400 flex flex-col justify-center items-center min-h-[280px] shadow-[4px_4px_0px_0px_#000]">
                ...
                <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider mt-2 max-w-xs mx-auto font-mono">
