...
export async function GET(request: Request, context: RouteContext) {
  ...
  try {
    ...
    const topAssistantLabel = recap.leaders.topAssistant
      ? `${recap.leaders.topAssistant.name} (${recap.leaders.topAssistant.assists})`
      : "—";

    trackOperationalEvent("recap_season_card_viewed", {
      seasonId,
    ...
    const diffSign = recap.goals.difference >= 0 ? "+" : "";
      ...
      <div class="card card-padded" style="padding:${isStories ? "30px 28px" : "28px 34px"};gap:0">
        ...
        <div style="display:flex;justify-content:space-between;align-items:flex-start">
          <div style="display:flex;flex-direction:column;gap:4px;flex:1;min-width:0">
            <div class="tracking-wide text-muted" style="font-size:13px;font-weight:600">SEASON RECAP</div>
            <div class="font-black" style="font-size:${isStories ? 42 : 48}px;line-height:1;letter-spacing:-0.02em">${esc(cut(recap.season.name, 30))}</div>
            <div class="text-muted font-medium" style="font-size:${isStories ? 20 : 22}px;margin-top:2px">${esc(teamLabel)} · ${recap.matches} jogo${recap.matches !== 1 ? "s" : ""}</div>
          </div>
          ${badgeHtml}
        </div>
        ...
        <div style="display:flex;flex-wrap:${isStories ? "wrap" : "nowrap"};gap:10px;margin-top:16px">
          ...
          <div class="record-chip" style="background:${diffBg};flex:0.7">
            <div class="num mono">${diffSign}${recap.goals.difference}</div>
        ...
        <div style="display:flex;gap:10px">
          ...
          <div class="stat-tile" style="width:100px;flex-shrink:0">
            ...
            <div class="value mono">${recap.discipline.yellowCards}</div>
          </div>
          <div class="stat-tile" style="width:100px;flex-shrink:0">
            <div class="label">Vermelhos</div>
            <div class="value mono">${recap.discipline.redCards}</div>
          </div>
        </div>
      </div>
    `;
